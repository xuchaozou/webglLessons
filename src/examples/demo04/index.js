...
const hook = ({gl}) => {
    ...
    const isCompileVertexShader = gl.getShaderParameter(vertexShader, gl.COMPILE_STATUS)
    ...
    const isCompileFragmentShader = gl.getShaderParameter(fragmentShader, gl.COMPILE_STATUS)
...
useEffect(() => {
    const linkEvent = e => {
        ...
        const a_Position = glRef.current.getAttribLocation(glRef.current.program, "a_Position");
        const a_PointSize = glRef.current.getAttribLocation(glRef.current.program , 'a_PointSize')
